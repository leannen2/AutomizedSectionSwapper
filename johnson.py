...
cycles = tuple(simple_cycles(graph))
print(cycles)

combinations = []
for g in cycles: 
    for i in range(len(combinations)):
        if any(vertex in g for gr in combinations[i] for vertex in gr):
            print('true')
            continue
        else:
            combinations[i].append(g)
    combinations.append([g])
# print(any(vertex in cycles[2] for vertex in cycles[1]))
print('combinations: ', combinations)
sortedCombinations = sorted(combinations, key = lambda x: sum(len(l) for l in x))
print('solution:', sortedCombinations[-1])